...
| R4 | **Leakage** — matches close off-platform, rail adoption stalls <40% | M | High | On-rail-only value design (doc 02 §2); rail priced under annoyance threshold; **collection doctrine** (fees prepaid/PSP-deducted/service-gated — never chased); auto-mode (PSP webhooks) removes attestation friction for adopters | Rail adoption <40% after 2 months → founder interviews with every leaked pair; accelerate PSP auto-mode rollout; adjust value/price before adding friction | Ops |
| R4b | **Booking-fee rejection** — ₡33k/side measures as too high vs. free FB groups | M | High | Pilot measures willingness before P1 switch-on (doc 03 §4); fee framed vs. broker month + scam risk; levers pre-agreed (size, timing split, side weighting — doc 02 §3) | Willingness <60% → pull levers in order; the floor is landlord-only 25% — never retreat to invoice-after revenue | Founders |
...
| R6 | **Incumbent inertia** — FB groups are free and sufficient for many users | M | High | Use those groups as an acquisition channel (doc 03 §3); differentiate through verification, templates and payment records | Activation strong but retention weak → interview users, validate which trust features matter and improve the two primary journeys before expanding scope | Founders |
...
2. Any **Critical**-impact risk must have its mitigation *done or in progress* — a planned
